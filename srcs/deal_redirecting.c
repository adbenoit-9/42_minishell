...
/*   Updated: 2020/10/20 15:50:16 by adbenoit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_isredirect(t_stock **cmd_lst, int *fd_in, int *fd_out)
{
	...
	{
		*fd_in = open((*cmd_lst)->next->tokens[0], O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR);
		if ((*cmd_lst)->input)
			*fd_out = open((*cmd_lst)->input, O_RDONLY);
	}
	else if ((*cmd_lst)->sep == LEFT)
		*fd_out = open((*cmd_lst)->next->tokens[0], O_RDONLY);
	else
		return (0);
	if (*fd_in != -1 && *fd_out != -1)
		return (1);
